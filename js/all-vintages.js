...
export function allVintages(regionS, headlineArray, listCheck) {
  d3.text(`/csv/${regionS}.csv`).then(function(data) {
      ...
      const htmlTemplateBasicData =`
          ...
          ${avVintageNV == "" ? "" 
            : 
          `<div>Vintage: </div><div>${avVintageNV}</div>
           <div>Method: </div><div>${avMethod}</div>
           <div>Sweetness style: </div><div>${avSweetness}</div>
          `
          }
      ...
      const htmlTemplateScores =`
        <div class="rs-summary">
          ...
          <div class="rs" title="${avRS}" data-content="${avRS}" style="width:${avRS}%"></div> <div class="rank">${avRANK}</div>
          <div class="qp-label">Appellation Quality/Price</div>
          <div class="qp" title="${avRS}" data-content="${avQP}" style="width:${avQP}%"></div> <div></div>
          ${avRS2 == "" ? "" : `${rs2Template}`}
          ${avRS3 == "" ? "" : `${rs3Template}`}
        </div>
      `
